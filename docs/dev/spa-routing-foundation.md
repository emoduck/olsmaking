...
### In Scope
- Plan route-based navigation for primary tabs: `/oversikt`, `/arrangement`, `/favoritter`, `/profil`.
...
- Keep `/` as default entry that resolves to arrangement-first landing behavior.
...
- Routing for future `Smakinger` section.
...
- Unknown SPA path should resolve to default primary route (`/arrangement`) in the client.
- Existing join form query params (`eventId`, `joinCode`) continue to work on arrangement flow.
...
- Integration: verify direct navigation and refresh for `/oversikt`, `/arrangement`, `/favoritter`, `/profil` in integrated host
